...
def grab_body(user):
	...
	return add_null(height), add_null(weight)
...
for user in USERS:
	...
	sleep_df = grab_sleep(user)
	heart_df = grab_hr(user)
	steps_df = grab_steps(user)
	height_df, weight_df = grab_body(user)
