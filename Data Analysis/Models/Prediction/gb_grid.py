...
grid = GridSearchCV(GradientBoostingClassifier(), params, scoring=scoring, refit=False, verbose=4, return_train_score=True, cv=3).fit(X_train, y_train)

results = pd.DataFrame(grid.cv_results_)
results.to_csv('gb_grid_results.csv')
print(results)
